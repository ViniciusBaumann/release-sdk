...
4. Create 2-8 vertical behavior tasks. A task contains its test, implementation, a small green
   ...
   When a D-XX, AC or task replaces, renames, reimplements or supersedes an existing path (module,
   function, endpoint, serializer field, wire key, setting, screen, hook, Celery task, command),
   grep the current code for that path and its tests, and make the task that introduces the
   replacement also delete them: list every superseded file/symbol/test in `removes:` and in
   `files:`, and say "delete X" in the action. Superseded code is never left "in case", behind a
   flag, as an alias, as a fallback or as a `# legacy` comment; its tests go with it.
...
7. Use one fullstack plan with ordered backend/frontend tasks; do not create dual pipelines.
   ...
   replacement and the removal of the superseded path and its tests in the same phase. `live`
   differs from `pre-launch` only in migration/rollout ceremony, never in legacy retention.
8. Write `{phase_dir}/{NN}-PLAN.md`, normally <=300 lines and always <=600.
</workflow>

<task_format>
### T01 — Observable slice
- files: [exact paths, including every path this task deletes]
- removes: [superseded files/symbols/tests this task deletes, or none]
- depends_on: []
- acceptance: [AC-01]
