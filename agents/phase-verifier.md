...
4c. Retention: apply the retained check below to every task `removes:` entry and to every D-XX/AC
   that replaces or supersedes an existing path. Findings are GAPs prefixed `RETAINED:`.
...
- `HOLLOW:` and `RETAINED:` findings are GAPS; neither is downgraded to a note.
</verdict>
...
<retained_check>
Superseded code that survives the phase is a GAP prefixed `RETAINED:`, whatever the maturity.
Per task `removes:` entry and per D-XX/AC that replaces, renames or reimplements a path:
- Grep for the old module, symbol, endpoint, serializer field, wire key, setting, screen or hook.
  Still defined, still routed, still imported by production code, or still emitted on the wire
  without a D-XX naming its consumer (`file:line`) is RETAINED.
- A test file, test case, fixture or factory whose only subject is the superseded behavior is
  RETAINED; green does not excuse it.
- A compatibility layer, alias, dual read/write, fallback to the old path, rollout flag, or
  `# legacy`/`deprecated`/`TODO remove` marker added or left by this phase without a D-XX is
  RETAINED.
- A SUMMARY sentence such as "kept for backward compatibility", "preserved for existing
  consumers" or "mantido por compatibilidade" without the D-XX is RETAINED, not a rationale.
Cite `file:line` and name what must be deleted.
</retained_check>
