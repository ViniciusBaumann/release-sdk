...
name: statusline
description: >
  Barra de status do Codex para projetos release-sdk: modelo + effort, projeto, branch, fase/
  ...
  resta sem perguntar, ou personalizar a UI do Codex. Codex only — sem efeito no Codex.
---
...
Só Codex: sob Codex (`PLUGIN_DATA` definido / sem `RELEASE_PLUGIN_ROOT`) imprima
"statusline: apenas Codex" e encerre sem tocar em nada.
...
1. Resolver o plugin: `PLUGIN_DIR="${RELEASE_PLUGIN_ROOT:-$(ls -d ${CODEX_HOME:-$HOME/.codex}/plugins/cache/release-sdk/release/*/ 2>/dev/null | tail -1)}"`.
